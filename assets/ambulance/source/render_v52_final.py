...
def import_vehicle(ren,rw,logo):
    ...
    for x in (-31,31):
        lens=cube((x,58,117),(17,8,2.8),(1.0,.90,.66),.06,.06,1,'headlight lens')
        lp=lens.GetProperty(); lp.SetAmbient(1); lp.SetDiffuse(.30); base.AddPart(lens)
...
def state(f,aspect='desktop'):
    ...
    wire_in=smoother(seg(f,94,108)); wire_out=1-smoother(seg(f,136,149)); wire=wire_in*wire_out
    white=smoother(seg(f,166,179))
...
def update(f,aspect,ren,veh,env,digital,road,L):
    ...
    headlight_phase=industrial*(1-smoother(seg(f,116,136)))
    for p in veh['beam_props']:
        p.SetOpacity(.055*headlight_phase*(1-white))
    ...
    for l,xx in L['head']:
        l.SetPosition(xx,1.05,vz+3.8); l.SetFocalPoint(xx,.62,vz+15); l.SetIntensity((1.25+2.8*wire)*headlight_phase*(1-white))
    if white > .995:
